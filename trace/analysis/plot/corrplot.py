...
def plot_corrplot(
    ...
    if line_kws is None:
        line_kws = dict(lw=1., color=trace.PAL_SET2[1])
    ...
    g = sns.jointplot(
        x, y, data=dataframe, kind='reg', space=0, color=trace.PAL_SET2[8], annot_kws=annot_kws,
        marginal_kws=marginal_kws, joint_kws=dict(lowess=lowess, scatter_kws=scatter_kws, line_kws=line_kws)
    )

    # - Extras
    if add_hline:
        g.ax_joint.axhline(0, ls='-', lw=0.1, c=trace.PAL_SET2[7])

    if add_vline:
        g.ax_joint.axvline(0, ls='-', lw=0.1, c=trace.PAL_SET2[7])
...
def plot_corrplot_discrete(x, y, z, dataframe, scatter_kws=None, line_kws=None):
    ...
    g.plot_marginals(
        _marginal_boxplot, palette=trace.PAL_BIN, data=dataframe, linewidth=.3, fliersize=1, notch=False, saturation=1.0,
        xs=x, ys=y, zs=z
    )

    sns.regplot(
        x=x, y=y, data=dataframe, color=trace.PAL_BIN[0], truncate=True, fit_reg=True, scatter_kws=scatter_kws, line_kws=line_kws, ax=g.ax_joint
    )
    sns.regplot(
        x=x, y=y, data=dataframe[dataframe[z] == 1], color=trace.PAL_BIN[1], truncate=True, fit_reg=False, scatter_kws=scatter_kws, ax=g.ax_joint
    )
    ...
    handles = [mpatches.Circle([.0, .0], .25, facecolor=c, label='Yes' if t else 'No') for t, c in trace.PAL_BIN.items()]
