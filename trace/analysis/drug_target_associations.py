...
def drug_count_barplot(df):
    plot_df = pd.Series({
        'All': len({(i, n, v) for i, n, v in df[trace.DRUG_INFO_COLUMNS].values}),
        'All unique': len({n for i, n, v in df[trace.DRUG_INFO_COLUMNS].values}),
        'W/Target': len({(i, n, v) for i, n, v in df.dropna(subset=['target'])[trace.DRUG_INFO_COLUMNS].values}),
        'W/Target unique': len({n for i, n, v in df.dropna(subset=['target'])[trace.DRUG_INFO_COLUMNS].values})
    }).rename('count').to_frame().reset_index()

    ax = sns.barplot('count', 'index', data=plot_df, color=trace.PAL_BIN[0], linewidth=.8, orient='h')
    ax.set_xlim(0, 1000)
...
def drug_beta_histogram(plot_df):
    sns.distplot(plot_df['beta'], color=trace.PAL_BIN[0], kde_kws=dict(cut=0, lw=1, zorder=1, alpha=.8), hist_kws=dict(alpha=.4, zorder=1), label='All', bins=30)
    sns.distplot(plot_df.query('target == 0')['beta'], color=trace.PAL_BIN[1], kde_kws=dict(cut=0, lw=1, zorder=3), hist_kws=dict(alpha=.6, zorder=3), label='Target', bins=30)

    beta_median = plot_df.query('fdr < 0.05 & target == 0')['beta'].median()
    plt.axvline(beta_median, c=trace.PAL_BIN[1], lw=.3, ls='--')
    plt.text(beta_median, 0, 'FDR < 5%', fontdict=dict(color='white', fontsize=3), rotation=90, ha='right', va='bottom')

    # plt.grid(True, color=trace.PAL_SET2[7], linestyle='-', linewidth=.1, alpha=1., zorder=0, axis='x')
    plt.axvline(0, c=trace.PAL_SET2[7], lw=.3, ls='-', zorder=0)
...
def recapitulated_drug_targets_barplot(df, thres_fdr, thres_beta):
    ...
    plot_df = pd.Series({
        'Non-significant': len({n for i, n, v in plot_df[trace.DRUG_INFO_COLUMNS].values}),
        'Significant': len({n for i, n, v in plot_df.query('fdr < {} & beta > {} & target < 1'.format(thres_fdr, thres_beta))[trace.DRUG_INFO_COLUMNS].values}),
    }).rename('count')

    for i, l in enumerate(plot_df.index):
        plt.bar(0, plot_df.loc[l], color=trace.PAL_BIN[i], label=l)
...
if __name__ == '__main__':
    ...
    def classify_drug_pairs(d1, d2):
        ...
        dsame = trace.is_same_drug(d1_id, d2_id, drugsheet=drugsheet) if d1_id in drugsheet.index and d2_id in drugsheet.index else d1_name == d2_name
    ...
    pal = dict(zip(*(order, sns.light_palette(trace.PAL_SET2[8], len(order) + 1).as_hex()[1:])))
