...
def drug_counts_hist(plot_df, by):
    ...
    if by == 'drug':
        for v, c in dtrace.PAL_DRUG_VERSION.items():
        ...
        sns.distplot(plot_df['count'], color=dtrace.PAL_SET2[8], kde=False, bins=20, hist_kws={'alpha': 1.})
...
def pca_pairplot(pca, by):
    ...
    g = sns.PairGrid(
        ...
        palette=dtrace.PAL_DRUG_VERSION if by == 'drug' else None
    )

    g = g.map_diag(plt.hist, color=dtrace.PAL_SET2[8] if by == 'sample' else None)

    if by == 'sample':
        cmap = sns.light_palette(dtrace.PAL_SET2[8], as_cmap=True)
...
if __name__ == '__main__':
    # - Imports
    # Samplesheet
    ss = pd.read_csv(dtrace.SAMPLESHEET_FILE, index_col=0)

    # Drug response
    d_response = pd.read_csv(dtrace.DRUG_RESPONSE_FILE, index_col=[0, 1, 2], header=[0, 1])
    ...
    g = sns.jointplot(
        'PC{}'.format(pci), 'growth_rate_median', data=plot_df, kind='reg', space=0, color=dtrace.PAL_SET2[8],
        marginal_kws=dict(kde=False), annot_kws=dict(stat='R'),
        joint_kws=dict(lowess=True, scatter_kws=dict(edgecolor='w', lw=.3, s=10, alpha=.6), line_kws=dict(lw=1., color=dtrace.PAL_SET2[1], alpha=1.))
    )

    g.ax_joint.axvline(0, ls='-', lw=0.1, c=dtrace.PAL_SET2[7], zorder=0)
    ...
    # -
    rnaseq = pd.read_csv(dtrace.RNASEQ_VOOM, index_col=0)
    ...
    sns.jointplot(X['AMOTL2'], y[X.index], color=dtrace.PAL_BIN[0])
    plt.show()

    sns.boxplot(X['BRAF_mut'], y[X.index], notch=True, palette=dtrace.PAL_BIN)
    plt.show()
    ...
    sns.jointplot(X['NOB1'], growth.loc[X.index, 'growth_rate_median'], color=dtrace.PAL_BIN[0])
    plt.show()
