...
if __name__ == '__main__':
    ...
    lm_df_crispr = lm_df_crispr.assign(DrugId=['|'.join(map(str, i)) for i in lm_df_crispr[dtrace.DRUG_INFO_COLUMNS].values])
    ...
    for i, s in enumerate(tsne_dict):
        for t in [0, 1]:
            ...
            axs[i].scatter(plot_df['P1'], plot_df['P2'], c=dtrace.PAL_BIN[t], label='W/Target' if t else 'No Target', edgecolors='white', lw=.3, alpha=.8, s=10)
