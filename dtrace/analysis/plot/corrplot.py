...
def plot_corrplot_discrete(x, y, z, dataframe, scatter_kws=None, line_kws=None):
    ...
    g.plot_marginals(
        _marginal_boxplot, palette=dtrace.PAL_BIN, data=dataframe, linewidth=.3, fliersize=1, notch=False, saturation=1.0,
        xs=x, ys=y, zs=z
    )

    sns.regplot(
        x=x, y=y, data=dataframe, color=dtrace.PAL_BIN[0], truncate=True, fit_reg=True, scatter_kws=scatter_kws, line_kws=line_kws, ax=g.ax_joint
    )
    sns.regplot(
        x=x, y=y, data=dataframe[dataframe[z] == 1], color=dtrace.PAL_BIN[1], truncate=True, fit_reg=False, scatter_kws=scatter_kws, ax=g.ax_joint
    )
    ...
    handles = [mpatches.Circle([.0, .0], .25, facecolor=c, label='Yes' if t else 'No') for t, c in dtrace.PAL_BIN.items()]
