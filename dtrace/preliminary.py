...
class Preliminary:
    ...
    def pairplot_pca_by_rows(cls, pca, hue='VERSION'):
        ...
        pal = None if hue is None else dict(v17=DTracePlot.PAL_DTRACE[2], RS=DTracePlot.PAL_DTRACE[0])
    ...
    def pairplot_pca_by_columns(cls, pca, hue=None, hue_vars=None):
        ...
        pal = None if hue is None else dict(Broad=DTracePlot.PAL_DTRACE[2], Sanger=DTracePlot.PAL_DTRACE[0])
        color = DTracePlot.PAL_DTRACE[2] if hue is None else None
    ...
    def pairplot_pca_samples_cancertype(cls, pca, cancertypes, min_cell_lines=20):
        ...
        pal = [DTracePlot.PAL_DTRACE[1]] + sns.color_palette('tab20', n_colors=len(order) - 1).as_hex()
    ...
    def corrplot_pcs_growth(cls, pca, growth, pc):
        ...
        line_kws = dict(lw=1., color=DTracePlot.PAL_DTRACE[0], alpha=1.)
        scatter_kws = dict(edgecolor='w', lw=.3, s=10, alpha=.6)
        joint_kws = dict(lowess=True, scatter_kws=scatter_kws, line_kws=line_kws)

        g = sns.jointplot(
            pc, 'growth', data=df, kind='reg', space=0, color=DTracePlot.PAL_DTRACE[2],
...
class DrugPreliminary(Preliminary):
    ...
    def histogram_sample(cls, samples_count):
        ...
        sns.distplot(df['count'], color=DTracePlot.PAL_DTRACE[2], hist=False, kde_kws=cls.HIST_KDE_KWS, label=None)
        sns.despine(top=True, right=True)
    ...
    def growth_correlation_histogram(cls, g_corr):
        ...
        plt.axvline(0, c=DTracePlot.PAL_DTRACE[1], lw=.1, ls='-', zorder=0)
    ...
    def growth_correlation_top_drugs(cls, g_corr, n_features=20):
        sns.barplot('corr', 'DRUG_NAME', data=g_corr.head(n_features), color=DTracePlot.PAL_DTRACE[2], linewidth=0)
        sns.despine(top=True, right=True)
...
class CrisprPreliminary(Preliminary):
    ...
    def corrplot_pcs_essentiality(cls, pca, crispr, pc):
        ...
        g = sns.jointplot(
            'count', pc, data=df, kind='reg', space=0, color=DTracePlot.PAL_DTRACE[2],
            marginal_kws=marginal_kws, annot_kws=annot_kws, joint_kws=joint_kws
        )
